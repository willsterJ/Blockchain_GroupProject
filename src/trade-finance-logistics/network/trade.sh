...
function replacePrivateKey () {
  ...
  if [ "$DEV_MODE" = true ] ; then
    ...
    if [ $(uname -s) == 'Darwin' ] ; then
      ...
      cd crypto-config/peerOrganizations/middlemanorg.trade.com/ca/
      PRIV_KEY=$(ls *_sk)
      cd "$CURRENT_DIR"
      sed -i '' "s/MIDDLEMAN_CA_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml
      CURRENT_DIR=$PWD
      cd crypto-config/peerOrganizations/warehouseorg.trade.com/ca/
      PRIV_KEY=$(ls *_sk)
      cd "$CURRENT_DIR"
...
# and four Peer Orgs (``SellerOrg``, ``BuyerOrg``, ``CarrierOrg`` & ``MiddlemanOrg``)
...
# (``peer0.sellerorg.trade.com`` & ``peer0.buyerorg.trade.com``).  Secondly, we point to
...
function generateChannelArtifacts() {
  ...
  if [ "$DEV_MODE" = false ] ; then
    ...
    echo
    echo "######################################################################"
    echo "#######  Generating anchor peer update for MiddlemanOrgMSP  ##########"
    echo "######################################################################"
    set -x
    configtxgen -profile $CHANNEL_PROFILE -outputAnchorPeersUpdate \
    ./channel-artifacts/MiddlemanOrgMSPanchors.tx -channelID $CHANNEL_NAME -asOrg MiddlemanOrgMSP -channelID $CHANNEL_NAME
    res=$?
    set +x
    if [ $res -ne 0 ]; then
      echo "Failed to generate anchor peer update for MiddlemanOrgMSP..."
      exit 1
    fi

    echo
    echo "#####################################################################"
    echo "#######  Generating anchor peer update for WarehouseOrgMSP  ##########"
    echo "#####################################################################"
    set -x
    configtxgen -profile $CHANNEL_PROFILE -outputAnchorPeersUpdate ./channel-artifacts/WarehouseOrgMSPanchors.tx -channelID $CHANNEL_NAME -asOrg WarehouseOrgMSP
    res=$?
    set +x
    if [ $res -ne 0 ]; then
      echo "Failed to generate anchor peer update for WarehouseOrgMSP..."
      exit 1
    fi
    echo
  fi
